...
export const AllTestsView: React.FC<AllTestsViewProps> = ({
  ...
  // Extract available unique course codes & weeks
  const availableCourses = Array.from(new Set(allTests.map((t) => t.courseCode)));
  const availableWeeks = Array.from(new Set(allTests.map((t) => t.week))).sort((a, b) => a - b);
      ...
      .filter((test) => {
        ...
        if (searchQuery.trim()) {
          ...
          const match =
            ...
            test.courseCode.toLowerCase().includes(q);
          if (!match) return false;
        }

        // Course filter
        if (courseFilter !== 'all' && !test.courseCode.includes(courseFilter)) {
          return false;
        }

        // Week filter
        if (weekFilter !== 'all' && test.week !== parseInt(weekFilter, 10)) {
          return false;
        }

        // Status filter
        if (statusFilter !== 'all') {
          if (test.status.toLowerCase() !== statusFilter.toLowerCase()) {
            return false;
          }
        }

        // Date range filter
        if (dateRangeFilter !== 'all' && test.date) {
          const now = new Date();
  ...
  }, [allTests, searchQuery, courseFilter, dateRangeFilter, weekFilter, statusFilter, sortBy]);
  ...
  return (
    <div className="w-full max-w-[1420px] mx-auto px-4 sm:px-6 py-4 animate-in fade-in duration-300">
        ...
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-xl sm:text-2xl font-bold text-[#43342a] tracking-tight">
              Upcoming Tests & Examinations
            </h1>
            <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-[#fcecee] text-[#8a4b53] border border-[#f5d7db]">
              {filteredTests.length} of {allTests.length} tests
            </span>
          </div>
          <p className="text-xs sm:text-sm text-[#8c7a6e] mt-1">
            Complete schedule, weight breakdown, and syllabus coverage for all semester tests.
          </p>
        </div>
      ...
      <div className="bg-[#fffefb] rounded-2xl border border-[#ede2d2] p-4 sm:p-5 shadow-xs mb-6 flex flex-col gap-3">
        ...
        {/* Bottom row: Filter Chips (Course, Date Range, Week, Status) */}
      ...
      {filteredTests.length === 0 ? (
        <div className="bg-[#fffefb] rounded-2xl border border-dashed border-[#ded2c0] p-12 text-center text-sm text-[#8c7a6e]">
          No tests match the selected filters. Try resetting the filters above.
        </div>
      ) : (
